...
public class MainActivity extends AppCompatActivity {
    ...
    TextView logText, logText2; //각각 로그와 저장된 이름데이터를 출력할 텍스트 뷰
    String btName, btName1 = null, btName2 = null, btName3 = null;
    String userName, userName1 = null, userName2 = null, userName3 = null;
    // 이름을 저장할 변수로 값이 저장되지 않으면 널값이 저장되있는다. 널 값이 저장되있다면 값이 등록되지 않은것으로 처리한다.

    EditText bt, user; //이름을 입력할 에딧 뷰
    TextFileManager mFileMgr; //파일 입출력

    int ch1 = 0; // 한번이라도 이름이 저장된적이 있는지 확인하는 변수
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // project 생성시 Minimum SDK 설정에서 API level 18 이상으로 선택해야
    ...
    public void onClick(View view) {
        ...
        if(view.getId() == R.id.regiBtn) {
            if((bt.getText().length() == 0)||(user.getText().length() == 0)) { // 이름이 작성 되지 않았을때 처리
                Toast.makeText(this, "BT 디바이스와 사용자 이름의 작성이 필요합니다. ", Toast.LENGTH_SHORT).show();
            }else{
                ch1 = 1; // 등록이 되었다는것을 알려주는 부분. 등록되어 있지 않았을 때와 등록후를 구별해주는 변수

                // EditText에 입력된 디바이스와 사용자 이름을 String 변수에 담음
                btName = bt.getText().toString();
                userName = user.getText().toString();

                // 3개의 대상자의 등록 여부에 따라 저장되지 않은 위치에 값을 저장하고 출력하는 부분과 3개의 위치에 모든값이 입력되었을때 처리
                if(btName1 == null) {
                    btName1 = btName; userName1 = userName;
                    logText2.setText("1 - "+ btName1 + " " + userName1);
                    Toast.makeText(this, "BT 디바이스와 사용자 이름 등록!!", Toast.LENGTH_SHORT).show();
                }
                else if(btName2 == null) {
                    btName2 = btName; userName2 = userName;
                    logText2.setText("1 - "+ btName1 + " " + userName1 +"  2 - "+ btName2 + " " + userName2);
                    Toast.makeText(this, "BT 디바이스와 사용자 이름 등록!!", Toast.LENGTH_SHORT).show();
                }
                else if(btName3 == null) {
                    btName3 = btName; userName3 = userName;
                    logText2.setText("1 - "+ btName1 + " " + userName1 +"  2 - "+ btName2 + " " + userName2 +"  3 - "+ btName3 + " " + userName3);
                    Toast.makeText(this, "BT 디바이스와 사용자 이름 등록!!", Toast.LENGTH_SHORT).show();
                }
                else {   Toast.makeText(this, "BT 디바이스와 사용자 이름을 작성할 공간이 없습니다. ", Toast.LENGTH_SHORT).show(); }


            }
        } else if(view.getId() == R.id.startMonitorBtn) {
            if(ch1 == 0) { // 이름이 하나도 등록 되지 않았을때 처리
                Toast.makeText(this, "BT 디바이스와 사용자 이름 등록이 되어있지 않습니다.", Toast.LENGTH_SHORT).show();
            }else{
                ...
                //모든 저장 정보 저장, 저장 되지 않았다면 널값이 전송
                intent.putExtra("BTName1", btName1);
                intent.putExtra("BTName2", btName2);
                intent.putExtra("BTName3", btName3);
